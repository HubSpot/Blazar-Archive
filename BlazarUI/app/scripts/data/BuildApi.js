...
import {extend, findWhere, max} from 'underscore';
...
class BuildApi {
  ...
  _getRepoBuildByParam(resp) {
    ...
    return findWhere(resp, {buildNumber: parseInt(this.params.buildNumber, 10)});
  }
  ...
  _getBuild() {
    ...
    return branchHistoryPromise.then((resp) => {
      ...
      const repoBuildId = this._getRepoBuildByParam(resp).id;
